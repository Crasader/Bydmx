...
public:
    ...
    void setInitTheseProperties(Ref *target, GMEnemy *enemy,float sacle);
    void setDelegate(Ref *target) { _delegate = target;}
    /*
     *  初始
     */
    ...
    void setRandEnemyValue(Ref *target, GMEnemy *enemy,float sacle);
    void setCurveValue(Ref *target,GMEnemy *enemy,CCPoint point,CCArray*array ,int direction, int row, float delay,float sacle);
    /*
     *  设置游动动画和死亡动画，以及选择角度
     */
...
//    void setRotationAndMove();
    void setRotationAndMove(Ref* target, SEL_CallFuncN selector);
