...
void GMBossPoseidon::showBoss(Ref *target,GMEnemy *enemy,CCPoint point,float scaleValue){
    _delegate = target;
...
//    m_pBossPoseidon->setInitTheseProperties(target,enemy,scaleValue);
    ...
    m_pBossPoseidon->createEnemyName(fishName);
    ...
    m_pBossPoseidon->setPosition(ccp(WINSIZE_W/2,WINSIZE_H-GetSize2(m_pBossPoseidon).height/2*scaleValue+50*Scale_X));
}
...
void GMBossPoseidon::showBossPoseidonBullet(CCNode *pNode){
    for (int i = 0; i < bulletNum; i ++) {
        ...
        tempPoint.x = rand()%(int)(WINSIZE_W/bulletNum-GetSize2(bulletEffect).width)+i*(WINSIZE_W/bulletNum);
...
void GMBossPoseidon::setTeleportMove(){
    if(teleportStatus == 1 || teleportStatus == 2){
        ...
        old.x = rand()%(int)(WINSIZE_W-GetSize2(m_pBossPoseidon).width*Scale_X)+GetSize2(m_pBossPoseidon).width/2*Scale_X;
