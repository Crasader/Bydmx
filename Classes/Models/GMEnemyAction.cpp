...
void GMEnemyAction::setInitTheseProperties(Ref *target, GMEnemy *enemy,float sacle)
{
    setDelegate(target);
    if (enemy) 
        setValue(enemy);
...
void GMEnemyAction::setRandEnemyValue(Ref *target, GMEnemy *enemy,float sacle)
{
//    _delegate = target;
//    setValue(enemy);
//    char fishName[40] = {0};
...
void GMEnemyAction::setCurveValue(Ref *target,GMEnemy *enemy,CCPoint point,CCArray*array ,int direction, int row, float delay,float sacle )
{
//    _delegate = target;
//    char fishName[40] = {0};
//    setValue(enemy);
...
void GMEnemyAction::addCurveMoveAtion(float dt)
{
    ...
    Vector<CCLagrange*> arrayOfActions;
    
    for (int i = 0; i < curveNum; i++) {
        if (a[i]) {
            arrayOfActions.pushBack(a[i]);
        }
        else{
            break;
        }
 ...
 //   curveAction= CCSequence::create(arrayOfActions,callFunc,NULL);
...
void GMEnemyAction::setRotationAndMove(Ref* target, SEL_CallFuncN selector)
{
