...
//    for(int i = 0 ; i < strangeNum; i++)
//    {
//        log("随机鱼的数量 %d (%d %d)",i,countStrangeNum[i][0],countStrangeNum[i][1]);
//    }
...
//    menuLayer->setAnger();
	...
	m_pLaserBullet->createBulletName("jiguang001.png");
...
void GMGameLayer::removeLaser(CCNode *pNode)
{
	...
	this->scheduleOnce(schedule_selector(GMGameLayer::showLaserReward,this), 1);
}
...
void GMGameLayer::spriteMoveFinished(Ref *object){
...
//                                CCScene *pScene = CCTransitionFade::create(0.0, GMWinLayer::scene(), Color3B::WHITE);
...
void GMGameLayer::checkUpadte(float t)
{
	...
	if (bossLayer) {
		...
		if (boss1) {
			if (bossLayer->getChildren().size()>0){
				boss1->_attackTime++;
...
void GMGameLayer::usedSkill1(Ref *object)
{
	if(getPauseStatus() == true) {
		return;
	}
...
void GMGameLayer::usedSkill2(Ref *object)
{
	if(getPauseStatus() == true) {
		return;
	}
...
void GMGameLayer::showBootModeFIshID16()
{
	...
	enemyAction->createEnemyName(fishName);
...
//    IOSLog("rotateDuration = %f",rotateDuration);
	CCRotateTo *rotateTo = CCRotateTo::create(0.2,angle);
...
void GMGameLayer::shootBossBullet()
{
	...
	if (tag == 11) {
		bullet = new GMBullet();
		bullet->createBulletName("bossgx012.png");
		GMPublicMethod::setRepeatAnimation(bullet, 12, 14, "bossgx", "png", 0.1,true);
	}
	else {
		bullet = new GMBullet();
		bullet->createBulletName("bossgx019.png");
...
void GMGameLayer::setOctopusShootBullet(CCPoint point ,float rotation)
{
	...
	if (bossOctopus) {
		...
		bullet->createBulletName("octopus_b_001.png");
...
//    }
	...
	GMBullet *bullet = new GMBullet();
	bullet->createBulletName("bullet_20_1_000.png");
...
void GMGameLayer::removeDialogLayer3(Ref *pSender){
