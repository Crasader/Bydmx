...
void GMBillingLayer::showGenuineValidationTips(){
    ...
    ZPLog("测试133312222");
    float validtionbgW = validtionbg->getContentSize().width;
    float validtionbgH = validtionbg->getContentSize().height;

    CCSprite *tips1 = setSprite("billingPic/charging024.png",ccp(validtionbgW/2,validtionbgH),ccp(0.5,1),validtionbg,1,1);

    CCPoint point = ccp(validtionbgW/2,validtionbgH-tips1->getContentSize().height+10);
    setSprite("billingPic/charging026.png",point,ccp(0.5,1),validtionbg,1,1);
    
    CCMenuItem *closeItem = CCMenuItemImage::create("billingPic/charging001.png", "billingPic/charging002.png", this, menu_selector(GMBillingLayer::closeAction,this));
    closeItem->setAnchorPoint(ccp(0,0));
    closeItem->setPosition(ccp(validtionbgW-100,validtionbgH-100));
    
    CCMenuItem *sureItem = CCMenuItemImage::create("billingPic/charging028.png", "billingPic/charging029.png", this, menu_selector(GMBillingLayer::sureAction,this));
    ...
    if(string)
        showLabelTTF(string->getCString(),CCSize(WINSIZE_W, 69),Color3B::WHITE);
#endif
}
...
void GMBillingLayer::showSecondaryConfirmationTips(int index){
    ...
    float purchaseBgH = purchaseBg->getContentSize().height;
    point = ccp(0,0);
    if(index == 0){
    ...
    CCMenuItem *sureItem= CCMenuItemImage::create("billingPic/charging028.png", "billingPic/charging029.png", CC_CALLBACK_1(GMBillingLayer::sureAction,this));
...
void GMBillingLayer::showThePurchaseOfEnergy(int type,int index){
    ...
    CCMenuItem *closeItem = CCMenuItemImage::create("billingPic/charging001.png", "billingPic/charging002.png", CC_CALLBACK_1(GMBillingLayer::closeAction,this));
    closeItem->setAnchorPoint(ccp(1,1));
    closeItem->setPosition(ccp(purchaseBgW,purchaseBgH));
    ...
    if(type == 1){
        if(index == 1)
            showLabelTTF("再等待30秒可以领取1000能量哦！",CCSize(WINSIZE_W, 69),Color3B::WHITE);
        else if(index == 2)
            showLabelTTF("再等待45秒可以领取5000能量哦！",CCSize(WINSIZE_W, 69),Color3B::WHITE);
    }else if(type == 2){
        int operatorsIndex = ZPJNIHelper::getOperators();
        CCString *string = NULL;
...
void GMBillingLayer::showWeaponUpgrades(int index){
    ...
    CCMenuItem *sureItem = CCMenuItemImage::create("billingPic/charging009c.png", "billingPic/charging009c2.png", CC_CALLBACK_1(GMBillingLayer::sureAction,this));
...
void GMBillingLayer::showPurchasingSkills(){
    ...
    CCMenuItem *sureItem = CCMenuItemImage::create("billingPic/charging034.png", "billingPic/charging035.png", CC_CALLBACK_1(GMBillingLayer::sureAction,this));
...
void GMBillingLayer::show91Shop(){
    ...
    CCMenuItem *item1 = CCMenuItemImage::create("billingPic/charging066.png", NULL, CC_CALLBACK_1(GMBillingLayer::sureAction,this));
    item1->setPosition(ccp((item1->getContentSize().width/2)+20,purchaseBgH/2));
    item1->setTag(1);
    point = ccp(item1->getContentSize().width,item1->getContentSize().height);
    setSprite("billingPic/charging067.png",point,ccp(1,1),item1,0,1);
    
    point = ccp(item1->getContentSize().width/2,item1->getContentSize().height/2);
    setSprite("billingPic/charging073.png",point,ccp(0.5,0.5),item1,0,1);
    
    CCMenuItem *item2 = CCMenuItemImage::create("billingPic/charging066.png", NULL, CC_CALLBACK_1(GMBillingLayer::sureAction,this));
    item2->setPosition(ccp(purchaseBgW/2,purchaseBgH/2));
    item2->setTag(2);
    
    point = ccp(item2->getContentSize().width,item2->getContentSize().height);
    setSprite("billingPic/charging068.png",point,ccp(1,1),item2,0,1);
    
    point = ccp(item2->getContentSize().width/2,item2->getContentSize().height/2);
    setSprite("billingPic/charging074.png",point,ccp(0.5,0.5),item2,0,1);
    
    CCMenuItem *item3 = CCMenuItemImage::create("billingPic/charging066.png", NULL, CC_CALLBACK_1(GMBillingLayer::sureAction,this));
    item3->setPosition(ccp(purchaseBgW/2+item1->getContentSize().width+20,purchaseBgH/2));
    item3->setTag(3);
    
    point = ccp(item3->getContentSize().width,item3->getContentSize().height);
    setSprite("billingPic/charging069.png",point,ccp(1,1),item3,0,1);
    
    point = ccp(item3->getContentSize().width/2,item3->getContentSize().height/2);
...
void GMBillingLayer::show91MobileView(){
    ...
    CCMenuItem *textFieldItem = CCMenuItemImage::create("91jifei/jifei008.png","91jifei/jifei008.png", CC_CALLBACK_1(GMBillingLayer::textFieldAction,this));
    textFieldItem->setAnchorPoint(ccp(0,0.5));
    textFieldItem->setPosition(ccp(30, purchaseBgH*0.5-20));
    //找好友代付
    CCMenuItem *friendItem = CCMenuItemImage::create("91jifei/jifei011.png","91jifei/jifei012.png", CC_CALLBACK_1(GMBillingLayer::getFriendAction,this));
    friendItem->setAnchorPoint(ccp(0,0.5));
    friendItem->setPosition(ccp(purchaseBgW/2+50, purchaseBgH*0.5-20));
    
    //获取验证码
    CCMenuItem *getCode = CCMenuItemImage::create("91jifei/jifei009.png","91jifei/jifei010.png", CC_CALLBACK_1(GMBillingLayer::getCodeAction,this));
...
//    CCString * string = CCString::create("共需支付话费 2 元(折合200点)\n请输入您的手机号码以获取验证码");
    ...
    textfield->setDimensions(textFieldItem->boundingBox().size.width,textFieldItem->boundingBox().size.height-20);
...
void GMBillingLayer::show91MobileView2(){
    ...
    //确认
    CCMenuItem *getCode = CCMenuItemImage::create("91jifei/jifei014.png","91jifei/jifei015.png", CC_CALLBACK_1(GMBillingLayer::sendCodeAction,this));
    getCode->setAnchorPoint(ccp(0.5,0.5));
    getCode->setPosition(ccp(purchaseBgW/2,0));
...
void GMBillingLayer::textFieldAction(Ref *object){
...
void GMBillingLayer::getFriendAction(Ref *object){
#if (JNIHELPER_INDEX == JNIHELPER_91_IOS)
    //PublicCPP::addPicker();
...
void GMBillingLayer::getCodeAction(Ref *object){
    if(textfield && strlen ( textfield->getString().c_str() ) > 0){
        _isClickClose = false;
#if (JNIHELPER_INDEX == JNIHELPER_91_IOS)
        billingIndex = GMPublicMethod::sharedPublicMethod()->getBillingIndex();
        
        GMPublicMethod::sharedPublicMethod()->setPhoneNumber(textfield->getString().c_str());
...
void GMBillingLayer::sendCodeAction(Ref *object){
#if (JNIHELPER_INDEX == JNIHELPER_91_IOS)
    if(textfield && strlen ( textfield->getString().c_str() ) > 0){
        billingIndex = GMPublicMethod::sharedPublicMethod()->getBillingIndex();
...
void GMBillingLayer::sureAction(Ref *pSender){
...
void GMBillingLayer::closeAction(Ref *pSender){
...
bool GMBillingLayer::onTouchBegan(cocos2d::CCTouch *pTouch, cocos2d::CCEvent *pEvent){
    if (m_pMenu) {
...
void GMBillingLayer::onTouchMoved(cocos2d::CCTouch *pTouch, cocos2d::CCEvent *pEvent){
    if (m_bTouchedMenu && m_pMenu) {
        m_pMenu->ccTouchMoved(pTouch, pEvent);
    }
}

void GMBillingLayer::onTouchEnded(cocos2d::CCTouch *pTouch, cocos2d::CCEvent *pEvent){
...
void GMBillingLayer::onTouchCancelled(cocos2d::CCTouch *pTouch, cocos2d::CCEvent *pEvent){
    if (m_bTouchedMenu && m_pMenu) {
